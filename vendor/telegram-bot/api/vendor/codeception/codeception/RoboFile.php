...
class RoboFile extends Tasks
{
    ...
    {
        $this->say("CODECEPTION RELEASE: ". Codecept::VERSION);
        $this->update();
        $this->buildDocs();
        $this->publishDocs();
    ...
    {
        $this->server();
        $this->taskSymfonyCommand(new Run('run'))
            ->arg('suite','coverage')
            ->run();
    }
    ...
    {
        if (strpos(Codecept::VERSION, self::STABLE_BRANCH) !== 0) {
            $this->say("The ". Codecept::VERSION." is not in release branch. Site is not build");
            return;
        }
    ...
    {
        $version = isset($opt['tag']) ? $opt['tag'] : Codecept::VERSION;
