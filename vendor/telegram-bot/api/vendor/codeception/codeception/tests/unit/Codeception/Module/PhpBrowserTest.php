...
class PhpBrowserTest extends TestsForBrowsers
{
    ...
    protected function setUp() {
        $this->module = new PhpBrowser(make_container());
        ...
        if (class_exists('GuzzleHttp\Url')) {
            ...
            $this->module->guzzle->getConfig('handler')->push(Middleware::history($this->history));
        }
    ...
    {
        $code = $this->module->executeInGuzzle(function(Client $client) {
            $res = $client->get('/info');
    ...
    public function testCookiesForDomain()
    {
        ...
        $client = new Client(['handler' => $handler, 'base_uri' => 'http://codeception.com']);
        $guzzleConnector = new Guzzle6();
